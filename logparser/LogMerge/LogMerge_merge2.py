...
class LogMerge:
    ...
    def createGroupClust(self, processed_df): 
        log_clusters_list = []        
        processed_df['lineID'] = processed_df.index.values
        for idx, row in processed_df.iterrows():
            tokens = row['GroupTokens'].copy()
            if len(row['idxDynamicTok']) != 0:
                for idxs, val in enumerate(row["idxDynamicTok"]):
                    if self.merge_special:
                        tokens[val] = self.mergeSpecialTok("".join(row['StaticTokList'][idxs]))
                    else:
                        tokens[val] = "".join(row['StaticTokList'][idxs])
    ...
    def outputResult(self, logfile):
        ...
        for logClust in self.log_clusterL:
            ...
            log_id_list = list(logClust.logIDL) if isinstance(logClust.logIDL, (list, tuple)) else []
            for logID in log_id_list:
