...
benchmark_dir = "../../benchmark/LogMerge/nmerge/"
benchmark_file = "LogMerge_threshold_nmerge"

n_mergeL = [2,3,4,5,6,7,8,9,10]
st_values = [setting['st'] for setting in SETTING_PARAMS.values() if 'st' in setting]
average_st = sum(st_values) / len(st_values) if st_values else 0
punctuationL = "[](){}=:,"
# st = [0.2, 0.25, 0.3, 0,35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9]
# punctuationL_lst= [
#     "[](){}",
#     "[](){}=:#",
#     "[](){}=:#,",
...
def main():
    for n in n_mergeL:
        result_file = os.path.join(benchmark_dir, benchmark_file+ "_" + str(n) + ".csv")
        if os.path.exists(result_file):
            os.remove(result_file)
        func.prepare_results(benchmark_dir, benchmark_file+ "_" + str(n) + ".csv")
        for name_data, data_setting in SETTING_PARAMS.items():
            parser = LogMerge.LogMerge(
                ...
                n_merge=n, st=average_st, punctuation_char=punctuationL, 
