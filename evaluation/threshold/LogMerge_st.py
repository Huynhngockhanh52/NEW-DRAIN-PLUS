...
benchmark_dir = "../../benchmark/LogMerge/st/"
benchmark_file = "LogMerge_st"

average_n = 4
punctuationL = "[](){}=:,"
stL = [0.2, 0.25, 0.3, 0,35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9]
# punctuationL_lst= [
#     "[](){}",
#     "[](){}=:#",
#     "[](){}=:#,",
...
def main():
    for st in stL:
        result_file = os.path.join(benchmark_dir, benchmark_file+ "_" + str(st) + ".csv")
        if os.path.exists(result_file):
            os.remove(result_file)
        func.prepare_results(benchmark_dir, benchmark_file+ "_" + str(st) + ".csv")
        for name_data, data_setting in SETTING_PARAMS.items():
            parser = LogMerge.LogMerge(
                ...
                n_merge=average_n, st=st, punctuation_char=punctuationL, 
